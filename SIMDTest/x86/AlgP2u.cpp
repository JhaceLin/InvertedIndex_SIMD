...
int GetX_Search_P_Old(IList &ilist, int e)
{
...
__m128i vindex = _mm_set_epi32(interval * 3, interval * 2, interval * 1, 0);
...
int GetX_Search_P(IList &ilist, int e)
{
    ...
    __m128i dst;
    x1 = _mm_set1_epi32(e);
    int preI, cmp, andOp, zeroPos;
    int l = ilist.start;
    for(; l + interval * 4 < n; l += interval * 4) {
        dst = _mm_i32gather_epi32(arr + l, vindex, 4);
        r = _mm_cmpgt_epi32(x1, dst);
        preI = _mm_movemask_ps(reinterpret_cast<__m128>(r));
        cmp = preI ^ 0xF;
        if(cmp) {
            andOp = 1;
            zeroPos = 0;
            ...
            i = l + zeroPos * interval;
            break;
        }
    }
    
    if(i != ilist.start) i -= interval;
...
void GetIntersection(int *result, int &resultCount)
{
    ...
    while(ilist[visit[0]].remainLen > 0) {
        ...
        for(i = 1; i < numQuery; i++) {
            ...
            /*
            int xOrder_Old = GetX_Search_P_Old(ilist[visit[i]], e);

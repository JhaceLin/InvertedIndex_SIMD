...
int start_align_rst = 0;
int *start_align_bindex = NULL;
...
void ReleaseSpace() {
    ...
    // Delete start_align_bindex
    delete[] start_align_bindex;
}

inline int GetOffset(void const *address) {
    long long t = reinterpret_cast<long long>(address);
    return t % 16LL;
}

void Get_start_align_rst() {
    int offset = GetOffset(rst);
    start_align_rst = 0;
    if(offset) {
        start_align_rst = (16 - offset) / 4;
    }
}

void Get_start_align_bindex() {
    if(start_align_bindex == NULL) {
        start_align_bindex = new int[numIndex]();
    }

    int offset;
    for(int i = 0; i < numIndex; i++) {
        offset = GetOffset(bindex[i]);
        if(offset) {
            start_align_bindex[i] = (16 - offset) / 4;
        }
    }
}

bool AlignmentTest(bool choice) {
    // True: bindex  False: rst
    long long address;
    if(choice) {
        for(int i = 0; i < numIndex; i++) {
            address = reinterpret_cast<long long>(bindex[i]);
            if (address % 16LL) {
                return false;
            }
        }
        ...
        address = reinterpret_cast<long long>(rst);
        if (address % 16LL) {
            return false;
        }
    }
...
int32x4_t zero_int32x4t = vmovq_n_s32(0);
...
void GetAnswer(int rstLen) {
    ...
    // 串行处理未对齐部分
    for(sect = 0; sect < start_align_rst; sect++) {
...
void GetIntersection() {
    ...
    int sect_align = 0;
    for(int i = 0; i < numQuery; i++) {
        int t = start_align_bindex[query[i]];
        if(t > sect_align) {
            sect_align = t;
        }
    }
    // 串行处理未对齐部分
    for(sect = 0; sect < sect_align; sect++) {
    ...
    // 并行处理对齐部分
    int32x4_t x1, x2;
    for(sect = sect_align; sect + 4 < rstLen; sect += 4) {
...
int main() {
    ...
    Get_start_align_bindex();

    /*bool bindexAlignment = AlignmentTest(true);
    if(bindexAlignment) {
        cout << "bindex is align" << endl;
    } else {
        cout << "bindex is not align" << endl;
    }*/
    ...
    /*bool rstAlignment = AlignmentTest(false);
    if(rstAlignment) {
        cout << "rst is align" << endl;
    } else {
        cout << "rst is not align" << endl;
    }*/
